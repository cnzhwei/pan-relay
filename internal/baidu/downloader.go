...
func Download(ctx context.Context, downloadURL string, customHeaders http.Header, outputPath string, opt DownloadOptions) error {
	...
	// If HEAD failed or gave no length, probe with GET Range: bytes=0-0
	if fileSize <= 0 {
	...
	// Single-threaded fallback if tiny or no range support
	if !acceptRanges || fileSize <= 8*1024*1024 || opt.Concurrency == 1 {
		fmt.Printf("Downloading %s directly from Baidu (single-threaded)...\n", filepath.Base(outputPath))
		...
		if gResp.StatusCode != http.StatusOK && gResp.StatusCode != http.StatusPartialContent {
			return fmt.Errorf("baidu server returned HTTP %d", gResp.StatusCode)
		}
		...
		fmt.Printf("[✓] Baidu download completed: %s\n", outputPath)
		return nil
	}
	...
	fmt.Printf("Downloading %s (%.2f MB) from Baidu Netdisk with %d threads...\n",
	...
	fmt.Printf("[✓] Baidu download completed successfully: %s\n", outputPath)
